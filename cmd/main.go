...
var (
	...
	defaultOperatorNamespace = "storagegrid-operator-system" // Default namespace for the operator, can be overridden by OPERATOR_NAMESPACE env variable
)
...
func main() {
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElectionID:       "storagegrid-operator.bedag.ch",
	...
	if os.Getenv("ENABLE_WEBHOOKS") != "false" {
		if err = (&s3webhook.S3TenantAccountValidator{}).SetupWebhookWithManager(mgr); err != nil {
			setupLog.Error(err, "unable to create webhook", "webhook", "S3TenantAccount")
			os.Exit(1)
		}
	}
	...
	if operatorNamespace == "" {
		setupLog.Error(nil, fmt.Sprintf("OPERATOR_NAMESPACE environment variable is not set, defaulting to %s", defaultOperatorNamespace))
	...
	if os.Getenv("ENABLE_WEBHOOKS") != "false" {
		if err = (&s3webhook.S3TenantValidator{}).SetupWebhookWithManager(mgr); err != nil {
	...
	if os.Getenv("ENABLE_WEBHOOKS") != "false" {
		if err = (&s3webhook.S3BucketValidator{}).SetupWebhookWithManager(mgr); err != nil {
	...
	if metricsCertWatcher != nil {
		setupLog.Info("Starting metrics certificate watcher")

		// Start the watcher immediately in a goroutine, independent of leader election
		// This ensures certificate reloading works on all pods, not just the leader
		go func() {
			if err := metricsCertWatcher.Start(ctx); err != nil {
				setupLog.Error(err, "metrics certificate watcher stopped with error")
			}
		}()
	}

	if webhookCertWatcher != nil {
		setupLog.Info("Starting webhook certificate watcher")

		// Register callback before starting watcher
		webhookCertWatcher.RegisterCallback(func(c tls.Certificate) {
			setupLog.Info("Webhook certificate reloaded!")
		})
		...
		go func() {
			if err := webhookCertWatcher.Start(ctx); err != nil {
				setupLog.Error(err, "webhook certificate watcher stopped with error")
			}
		}()
	}
