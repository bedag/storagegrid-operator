...
type policyReconcileContext struct {
	ObjectUpdated bool
	DoRequeue     bool
	Policy        *s3v1alpha1.S3Policy
}

// +kubebuilder:rbac:groups=s3.bedag.ch,resources=s3policies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=s3.bedag.ch,resources=s3policies/status,verbs=get;update;patch
// +kubebuilder:rbac:groups=s3.bedag.ch,resources=s3policies/finalizers,verbs=update
// +kubebuilder:rbac:groups="",resources=events,verbs=create;patch

func (r *S3PolicyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := log.FromContext(ctx).WithValues("s3policy", req.NamespacedName)
	log.V(1).Info("Starting reconciliation")

	// initialize reconcile context
	rctx := &policyReconcileContext{
		Policy:        &s3v1alpha1.S3Policy{},
		ObjectUpdated: false,
		DoRequeue:     false,
	}

	// fetch the S3Policy instance
	if err := r.Get(ctx, req.NamespacedName, rctx.Policy); err != nil {
		log.Error(err, "Failed to get S3Policy")
		return ctrl.Result{}, client.IgnoreNotFound(err)
	}

	log.V(1).Info("Successfully fetched S3Policy", "name", rctx.Policy.Name)
...
func (r *S3PolicyReconciler) doReconcile(ctx context.Context, rctx *policyReconcileContext) error {
...
func (r *S3PolicyReconciler) finalize(ctx context.Context, pol *s3v1alpha1.S3Policy) error {
...
func (r *S3PolicyReconciler) reconcileFinalizerAndDelete(ctx context.Context, rctx *policyReconcileContext) error {
	log := log.FromContext(ctx)
	...
	if rctx.Policy.DeletionTimestamp.IsZero() {
		...
		if !controllerutil.ContainsFinalizer(rctx.Policy, PolicyFinalizer) {
			...
			log.V(1).Info("Adding finalizer to policy")
		...
		log.V(1).Info("Policy is being deleted, checking for finalizer")
		if controllerutil.ContainsFinalizer(rctx.Policy, PolicyFinalizer) {
			...
			log.V(1).Info("Removing finalizer from policy")
		...
		log.V(1).Info("Policy is being deleted, no finalizer present")
		return nil
	}
...
func (r *S3PolicyReconciler) reconcileRenderedPolicy(ctx context.Context, rctx *policyReconcileContext) error {
...
func (r *S3PolicyReconciler) setCondition(pol *s3v1alpha1.S3Policy, condType string, status metav1.ConditionStatus, reason string, message string) {
